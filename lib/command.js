(function () {
    ...
    var notSources = {};
    var watchedDirs = {};
    program.version(packagejson.version).usage('[options] <scripts ...>').option('-w, --watch', 'watch for file changes').option('-b, --bare', 'compile without a top-level function wrapper').option('-o, --out <directory>', 'set output directory', String).parse(process.argv);
    if (program.out) {
    ...
    function compileScript(file, input, base) {
        ...
        try {
            js = fancyscript.compile(input, options);
